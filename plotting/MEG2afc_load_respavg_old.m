...
%     addpath(fullfile(MATLABPATH, 'tools', 'stats'))
%     addpath(fullfile(MATLABPATH, 'tools', 'fieldtrip-20161220')) %inc JJ edit ft_artifact_zvalue
%     ft_defaults
%     rmpath(genpath(fullfile(MATLABPATH, 'MEG_HH_analysis', '.git')))
% end
...
faxis_all = cat(1,faxis{:});
taxis_all = cat(1,taxis{:});
...
% stim and resp dims
respavg.dat = nan(length(SUBJ),length(respavg.label),max(faxis_lengths), max(taxis_lengths),4, 4, 2, 3, 3, 'single'); % subj chan freq time ipharm, idiff, itrig, ifreq, ilatr
respavg.pow = nan(length(SUBJ),length(respavg.label),max(faxis_lengths), max(taxis_lengths),4, 4, 2, 3, 3, 'single'); % subj chan freq time ipharm, idiff, itrig, ifreq, ilatr
...
for isub = 1:length(respavg.SUBJ)
    for itrig = 1%:2
        for ifreq = 1 %1:3
            ...
            PREIN = fullfile(basepath, 'projectdata/MEG2afc/freq', analysistype{ifreq}, trigger_leg{itrig}, baselinetype, phaselock_type, filesep );
            ...
            respavg.dat(isub,:,1:length(frind{ifreq}),1:length(tind{itrig}), 1:2, 1:3, itrig, ifreq, 1) = ...
                squeeze(nanmean(dat.subjrespavg(:,frind{ifreq}, tind{itrig},:, :, :, 3,3), 5)); % 1 = modulation, avg over ses
            % raw pow:
            respavg.pow(isub,:,1:length(frind{ifreq}),1:length(tind{itrig}), 1:2, 1:3, itrig, ifreq, 1) = ...
                squeeze(nanmean(dat.subjpowavg(:,frind{ifreq}, tind{itrig},:, :, :, 3,3), 5)); % 1 = modulation, avg over ses
            
            % lateralization wrt resp:
            sensordat = squeeze(nanmean(dat.subjrespavg(:,frind{ifreq}, tind{itrig},:, :, :, 3, 1:2), 5)); % keep left and right bp, 6 dims remain
            ...
            respavg.dat(isub,:,1:length(frind{ifreq}),1:length(tind{itrig}), 1:2, 1:3, itrig, ifreq, 2) = temp; % L channels are nan
            
            %%%
            % lateralization wrt choice:
            % dims subjrespavg(:,:,:, ipharm, imotor, idiff, istim, iresp)
            sensordat = squeeze( dat.subjrespavg(:,frind{ifreq}, tind{itrig},:, :, :, 3, 1:2) ); % dims: chan freq time drug ses diff resp
            ...
            respavg.dat(isub,:,1:length(frind{ifreq}),1:length(tind{itrig}), 1:2, 1:3, itrig, ifreq, 3) = temp; % L channels are nan
           
            %%%
            % lateralization wrt stimulus location:
            sensordat = squeeze(nanmean(dat.subjrespavg(:,frind{ifreq}, tind{itrig},:, :, :, 1:2, 3), 5)); % keep left and right stim, 6 dims remain
            ...
            respavg.dat(isub, :, 1:length(frind{ifreq}), 1:length(tind{itrig}), 1:2, 1:3, itrig, ifreq, 4) = temp; % L channels are nan
...
respavg.pow(:,:,:,:, 4,:,:,:,:) = (respavg.pow(:,:,:,:, 1,:,:,:,:) - respavg.pow(:,:,:,:, 2,:,:,:,:)) ./...
    respavg.pow(:,:,:,:, 2,:,:,:,:); % drug contrast


respavg.dimord = 'subj_chan_freq_time_drug_diff_trig_freqrange_latr'; % subj chan freq time ipharm, idiff, itrig, ifreq, ilatr
