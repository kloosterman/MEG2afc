...
% SUBJ_inc = [1:5, 7:9, 11:13, 15:21]; % NK nrs! NK14 out: 3 orders of magnitude bigger meg responses
...
switch tiletype
  case 'freq_together'
    ...
    for imod = 5 %1:4 % 2 is latr %  2=stim, 3=button, 4 = prevbutton
      curfreq = {};
      for idrug = 1:2 %1:length(behavnames)
        for ifreq = 1:2
          curfreq{idrug,ifreq} = megdat.freq(idrug,imod,1,ifreq,idiff);
          curfreq{idrug,ifreq}.powspctrm = curfreq{idrug,ifreq}.powspctrm(incsubj,:,:);
        end
        cfg = [];
        cfg.appenddim = 'freq';
        freqapp{idrug} = ft_appendfreq(cfg, curfreq{idrug,:});
        freqapp{idrug} = rmfield( freqapp{idrug}, 'time');
        freqapp{idrug}.dimord = 'subj_chan_freq';
      end
      %% plot
      if ismac
        freq_drug = freqapp{1};
        ...
        cfg.showlegend = 'yes';
        figure; subplot(131)
        ft_singleplotER(cfg, freq_drug, freq_plac );
        ax=gca; ax.YScale = 'log'; %ax.YLim = [1.0000e-30 1.0000e-28];
        subplot(132)
        semilogy(freq_drug.freq, squeeze(mean(freq_drug.powspctrm,2)))
        ax=gca; ax.YScale = 'log'; ax.YLim = [1.0000e-30 1.0000e-28];
        subplot(133)
        semilogy(freq_plac.freq, squeeze(mean(freq_plac.powspctrm,2)))
        ax=gca; ax.YScale = 'log'; ax.YLim = [1.0000e-30 1.0000e-28];
      end
      %%
      cfg=[];
      ...
      freq = ft_math(cfg, freqapp{:})
      freq0 = freq;
      freq0.powspctrm = zeros(size(freq0.powspctrm));
      
      cfg = [];
      cfg.method           = 'montecarlo';
      %         cfg.statistic        = 'ft_statfun_correlationT';  %depsamplesT ft_statfun_correlationT_corrcol
      cfg.statistic        = 'depsamplesT';  %depsamplesT ft_statfun_correlationT_corrcol
  ...
  case 'timefreq_together'
    ...
    disp 'stats'
    stat = {};
    for idrug = [2, 4] % 2 plac, 4 atx-plac
