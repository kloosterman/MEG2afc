...
% % % % % ft_defaults
cd('/Users/kloosterman/gridmaster2012/kloosterman/MATLAB/MEG_HH_analysis/plotting')
...
%% plotting: 2AFC dva drug vs placebo
close all
SAV=0;
...
transparent = 1;
linecols = {'r', 'b'; '--r', '--b'; 'r', 'b'};
TYP = '2afc';
% for isoi = 1%:2% 1:length(SOINsel) %8:10 %[4, 7] %1:length(sois)
for isoi = 3 %[1,3]
    for idrug = 1:2 % 1:2 %4 % [1:2,4] % 1:2
        
        figure;    iplot=0; hold on
        set(gcf, 'Position', [0 -200 375*3 210*4])
        for idva= 1 % [1,3:5] %1:4
            iplot=iplot+1; subplot(2,2,iplot); hold on
            ctr=0; clear h; clear legtxt
            for idiff = 1:2 %3%1:4
                for imotor = 3 %1:2 %1:2%:3;
                    ctr=ctr+1;
                    dum = squeeze(nanmean(respavg(:,idva,isoi, tind,idrug, imotor, idiff, istim, iresp ))); %average over subj
                    dumstd = squeeze(nanstd(respavg(:,idva,isoi, tind,idrug, imotor, idiff, istim, iresp ))) / sqrt(nsub); %average over subj
%                     h(ctr) = shadedErrorBar(taxis, dum, dumstd, linecols{imotor,idrug}, transparent);
                    h(ctr) = shadedErrorBar(taxis, dum, dumstd, linecols{imotor,idiff}, transparent);
%                     legtxt{ctr} = sprintf('%s %s', motor_conds{imotor}, pharm_conds{idrug});
                    legtxt{ctr} = sprintf('%s %s', motor_conds{imotor}, diff_conds{idiff});
                    title({sprintf('%s %slocked', diff_conds{idiff}, trigger) ;
                        sprintf('%s %s %s', dvaleg{idva}, sens.leg{isoi} )}); % 'FontWeight','bold'
                    hold on
...
%                         ylim([0.75 0.9])
                    ...
                    if showstats
                        ...
                        THR=1;
                        BARWID = 5;
                        pval = poolstat{idva, idrug, idiff}.prob';   SIGCOL = get(h(idrug).mainLine , 'Color');
                        curylim = get(gca, 'YLIM'); BARPOS = curylim(1) + diff(curylim)/2;
                    ...
                    legend([h.mainLine], legtxt, 'Location', 'NorthEast'); legend BOXOFF
        ...
        if SAV
            ...
            outfile = sprintf( '%s%sdva_%slocked_%s_%s', outpath, filesep, trigger, TYP,  diff_conds{idiff} ); % motor_conds{imotor}, pupilconds{ipup}
            disp(outfile)
...
%% Plot dva for drug and plac for session 1:4 separately
...
for idiff = 3%1:2%1:4
    for isoi=[1,3] %1:3 %3 %1:3
        ...
        for idva = [1, 3, 5, 2]% [3,5,4] %[3,5] %[1, 3:5] % 1:3%:4
            ...
            for ises = 1:4 %1:2%:2%:2
                ...
                if plotdrug_or_motor == 1
                    for idrug = 1:2
                        ...
                        dum = squeeze(nanmean(respavg_ses(subj_drug_ind, idva,isoi, tind, ises, idiff, istim, iresp ))); %average over subj
                        %                     dum = squeeze(mean(respavg(:,idva,isoi, tind,idrug, imotor, idiff, istim, iresp ))); %average over subj
                        dumstd = squeeze(nanstd(respavg_ses(subj_drug_ind, idva,isoi, tind, ises, idiff, istim, iresp ))) / sqrt(length(subj_drug_ind)); %average over subj
                        
                        h(ctr) = shadedErrorBar(taxis, dum, dumstd, linecols{1,idrug}, transparent);
                        legtxt{ctr} = sprintf('%s', pharm_conds{idrug});
                    end
                else
                    for imotor = 1:2
                        ctr=ctr+1;
                        ...
                        h(ctr) = shadedErrorBar(taxis, dum, dumstd, linecols{1,imotor}, transparent);
                        legtxt{ctr} = sprintf('%s', motor_conds{imotor});
                    end
                end
                
                title({sprintf('%s %s %slocked', ses_conds{ises}, diff_conds{idiff}, trigger) ;
                    sprintf('%s %s %s', dvaleg{idva}, sens.leg{isoi} )}); % 'FontWeight','bold'
                hold on
...
%                 ylim([0.7 0.95])
                ...
                if showstats
                    ...
                    THR=0.05
                    BARWID = 5;
                    %                 pval = poolstat{idva, idrug, idiff}.prob';
                    pval = poolstat{ises, idva, idrug, isoi}.prob';
%                     SIGCOL = get(h(idrug).mainLine , 'Color');
                    SIGCOL = [0 0 0];
                    curylim = get(gca, 'YLIM'); BARPOS = curylim(1) + diff(curylim)/2;
        ...
        if SAV
            outpath = fullfile(PREOUT, 'dva');
            warning off; mkdir(outpath); warning on
            outfile = sprintf( '%s%sdva_%slocked_%s_%s_%s_%s_persession', outpath, filesep, trigger, TYP, sens.leg{isoi}, diff_conds{idiff}, dvaleg{idva}, drug_motor_leg{plotdrug_or_motor} ); % motor_conds{imotor}, pupilconds{ipup}
            disp(outfile)
            export_fig(outfile, '-pdf', '-transparent', '-depsc', '-nocrop') %'-png',  '-pdf',
        end;
    end %
...
%% Barplots mean dva in time window
...
for idiff = 3%1:2%1:4
    ...
    for idva = [1, 3, 5, 2]% [3,5,4] %[3,5] %[1, 3:5] % 1:3%:4
        ...
        for isoi= [1,3] %1:3 %3 %1:3
            dum = []; dumstd = [];
            iplot=iplot+1; subplot(4,2,iplot); hold on
            ctr=0; clear h; clear legtxt
            ...
            for ises = 1:4 %1:2%:2%:2
                if plotdrug_or_motor == 1
                    for idrug = 1:2
                        ctr=ctr+1;
                        subj_drug_ind = find(drug_by_order_mat(:,ises)==idrug);
                        dum(ises, idrug) = squeeze(nanmean(respavg_tim(subj_drug_ind, idva,isoi, ises, idiff, istim, iresp ))); %average over subj
                        dumstd(ises, idrug) = squeeze(nanstd(respavg_tim(subj_drug_ind, idva,isoi, ises, idiff, istim, iresp ))) / sqrt(length(subj_drug_ind)); %average over subj                        
                    end
...
%                         dumstd = squeeze(nanstd(respavg_ses(subj_motor_ind, idva,isoi, tind, ises, idiff, istim, iresp ))) / sqrt(length(subj_motor_ind)); %average over subj
                        ...
                        dum(ises, imotor) = squeeze(nanmean(respavg_tim(subj_motor_ind, idva,isoi, ises, idiff, istim, iresp ))); %average over subj
                        dumstd(ises, imotor) = squeeze(nanstd(respavg_tim(subj_motor_ind, idva,isoi, ises, idiff, istim, iresp ))) / sqrt(length(subj_motor_ind)); %average over subj
            ...
            for ises = 1:4
                if plotdrug_or_motor == 1
                    dat1 = squeeze(respavg_tim( find(drug_by_order_mat(:,ises)==1), idva,isoi, ises, idiff, istim, iresp ));
                    dat2 = squeeze(respavg_tim( find(drug_by_order_mat(:,ises)==2), idva,isoi, ises, idiff, istim, iresp ));
                else
                    dat1 = squeeze(respavg_tim( find(motor_by_order_mat(:,ises)==1), idva,isoi, ises, idiff, istim, iresp ));
                    dat2 = squeeze(respavg_tim( find(motor_by_order_mat(:,ises)==2), idva,isoi, ises, idiff, istim, iresp ));
                end
...
%                 pvals(ises) = randtest1d(dat1, dat2, 0, 1000);
            ...
            title({sprintf('mean dva from %g - %g, %s-locked', TIMLO, TIMHI, trigger ) ;
...
%                 ylim([0.25 0.7])
    ...
    if SAV
        outpath = fullfile(PREOUT, 'dva');
        warning off; mkdir(outpath); warning on
        outfile = sprintf( '%s%stimewin_dva_%slocked_%s_%s_%s_persession', outpath, filesep, trigger, TYP, sens.leg{isoi}, diff_conds{idiff},  drug_motor_leg{plotdrug_or_motor} ); % motor_conds{imotor}, pupilconds{ipup}
        disp(outfile)
        export_fig(outfile, '-pdf', '-transparent', '-depsc', '-nocrop') %'-png',  '-pdf',
    end;
